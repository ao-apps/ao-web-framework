	...
	protected void printJavaScriptIncludes(WebSiteRequest req, HttpServletResponse resp, ChainWriter out, WebPage page) throws IOException, SQLException {
		...
		if (O != null) {
			if (O instanceof String[]) {
				...
				int len = SA.length;
				for (int c = 0; c < len; c++) {
					out.print("    <script type='text/javascript' src='").encodeXmlAttribute(req.getEncodedURLForContextURL('/'+SA[c], null, false, resp)).print("'></script>\n");
				}
				...
				out.print("    <script type='text/javascript' src='").encodeXmlAttribute(req.getEncodedURLForContextURL('/'+O.toString(), null, false, resp)).print("'></script>\n");
			}
		}
	}
}
